...
public class GestionInformacionDelegate {

	private static final Logger logger = Logger.getInstance(GestionInformacionDelegate.class);

	private static final String REPORTE_EJB = "ReporteEJB";

	private static final String CORREO_EJB = "EnvioCorreoEJB";

	private static IEnvioCorreoRemote getEnvioCorreoService() throws SIGEP2SistemaException {
		IEnvioCorreoRemote commonService = null;
		try {
			ServiceLocator<IServiceRemote> serviceLocator = ServiceLocator.getInstance();
			String pakageClassName = IEnvioCorreoRemote.class.getName();
			commonService = (IEnvioCorreoRemote) serviceLocator.getService(ConfigurationBundleConstants.getEJBAppName()
					+ "/" + ConfigurationBundleConstants.getEJBModuleName() + "/" + CORREO_EJB + "!" + pakageClassName);
	...
	public static void enviarMail(String asunto, String cuerpoP, Object desde, List<?> aList, List<?> ccList)
			throws SIGEP2SistemaException {
		String msg = "static void enviarMail(String asunto, String cuerpoP, Object desde, List<?> aList, List<?> ccList)";
		try {
			IEnvioCorreoRemote service = getEnvioCorreoService();
			service.enviarMail(asunto, cuerpoP, desde, aList, ccList);
		} catch (NullPointerException e) {
			logger.error(msg, e);
			throw new SIGEP2SistemaException(
					MessagesBundleConstants.getStringMessagesBundle(MessagesBundleConstants.DLG_PROCESO_FALLIDO));
	...
	public static void enviarMail(String asunto, String cuerpo, Object desde, List<?> a, List<String> adjuntos,
			List<?> cc) throws SIGEP2SistemaException {
		String msg = "static void enviarMail(String asunto, String cuerpo, Object desde, List<?> a, List<String> adjuntos, List<?> cc)";
		try {
			IEnvioCorreoRemote service = getEnvioCorreoService();
			service.enviarMail(asunto, cuerpo, desde, a, adjuntos, cc);
		} catch (NullPointerException e) {
			logger.error(msg, e);
			throw new SIGEP2SistemaException(
					MessagesBundleConstants.getStringMessagesBundle(MessagesBundleConstants.DLG_PROCESO_FALLIDO));
